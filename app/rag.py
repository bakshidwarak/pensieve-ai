...
def _build_rag_graph(data_dir: str) -> "CompiledGraph":
    ...
    try:
        from langchain_openai import OpenAIEmbeddings  # Using OpenAI instead of Together
        ...
        traceback.print_exc()
        # Create empty retriever as fallback
        qdrant_vectorstore = None

    retriever = qdrant_vectorstore.as_retriever() if qdrant_vectorstore else None
    ...
    # Using OpenAI instead of Together
    from langchain_openai import ChatOpenAI
    llm = ChatOpenAI(
        model="gpt-4o-mini",
        temperature=0,
